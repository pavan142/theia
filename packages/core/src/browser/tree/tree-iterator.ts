...
export class PreOrderTreeIterator extends AbstractTreeIterator {
    ...
    protected iterator(root: TreeNode): IterableIterator<TreeNode> {
        return preOrder(root, this.children.bind(this));
    }
...
export class BreadthFirstTreeIterator extends AbstractTreeIterator {
    ...
    protected iterator(root: TreeNode): IterableIterator<TreeNode> {
        return breadthFirst(root, this.children.bind(this));
    }
 ...
 *       +---2.1
 * ```
 * When selecting `1.2` as the root, the normal `PreOrderTreeIterator` would stop on `1.2` as it does not have children,
 ...
 */
export class TopToBottomTreeIterator extends PreOrderTreeIterator {
 ...
 * while traversing up the tree hierarchy in an inverse pre-order fashion. This is the counterpart of the `TopToBottomTreeIterator`.
 */
export class BottomToTopTreeIterator extends AbstractTreeIterator {
 ...
 */
function* preOrder<T>(root: T, children: (node: T) => T[] | undefined, include: (node: T) => boolean = () => true): IterableIterator<T> {
    const stack: T[] = [];
    stack.push(root);
 ...
 */
function* breadthFirst<T>(root: T, children: (node: T) => T[] | undefined, include: (node: T) => boolean = () => true): IterableIterator<T> {
    const queue: T[] = [];
    queue.push(root);
