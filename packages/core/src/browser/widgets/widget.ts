...
export function addKeyListener<K extends keyof HTMLElementEventMap>(element: HTMLElement, keysOrKeyCodes: KeysOrKeyCodes, action: (event: KeyboardEvent) => void, ...additionalEventTypes: K[]): Disposable {
    ...
    const isAcceptedKeyCode = (actual: KeyCode) => toKeyCodes(keysOrKeyCodes).some(k => k.equals(actual));
...
const toKeyCode = (keyOrKeyCode: Key | KeyCode) => keyOrKeyCode instanceof KeyCode ? keyOrKeyCode : KeyCode.createKeyCode({ first: keyOrKeyCode });
const toKeyCodes = (keysOrKeyCodes: KeysOrKeyCodes) => {
